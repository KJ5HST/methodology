...
The entries below name skills considered during the v2.6 audit (or surfaced after) that were *deliberately* not cited, with a one-line rationale each. This subsection exists to (a) close the door on future re-litigation and (b) make the audit decisions discoverable from this index rather than only from the underlying [`2026-05-02-mattpocock-skills-evaluation.md`](https://github.com/KJ5HST/methodology/blob/main/docs/audits/2026-05-02-mattpocock-skills-evaluation.md). That audit ran inside a different methodology instance and argued several of its rejections from *that* instance's numbered learnings, which are not this framework's; the rows below are stated on grounds a reader of this corpus can check, and the audit keeps its original wording as a dated record.
...
| `/to-issues` | Pocock (`engineering/to-issues`) | Not cited: the methodology owns no issue-*creation* procedure to point a reader at. What it owns is the *bar* an issue must clear — no issue reaches `ready-for-agent` without explicit acceptance criteria ([`DEVELOPMENT_WORKSTREAM.md`](docs/methodology/workstreams/DEVELOPMENT_WORKSTREAM.md) §Issue Lifecycle, transition rule 2) — and that bar binds whatever produced the issue. |
...
| `/caveman` | Pocock (`productivity/caveman`) | Stylistic compression aimed at token count. The methodology constrains a handoff by *content*, not by length: all six Minimum Handoff Requirements are mandatory and the next session scores the result ([`SESSION_RUNNER.md`](SESSION_RUNNER.md) §Phase 3D), and failure mode #15 names the *thin* handoff — not the long one — as the failure. Compression that drops any of the six fails that gate; compression that keeps them saves little. There is no length rule here to trade against. |
| `/zoom-out` | Pocock (`engineering/zoom-out`) | Single-prompt context elevation. The methodology's equivalent is §Sub-Agent Dispatch Pattern in [`INHERITED_CODEBASE_FAMILIARIZATION_CAMPAIGN.md`](docs/methodology/workstreams/INHERITED_CODEBASE_FAMILIARIZATION_CAMPAIGN.md) — a bounded sub-agent per call-graph cluster, returning a locked deliverable schema that requires file:line evidence for every assertion. Narrower in scope (codebase takeover) and stricter about what comes back. |
