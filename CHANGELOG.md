...
- **Change:** `methodology_dashboard.py` (both byte-identical twins, `tools/` + `starter-kit/`) now
  ...
  **no hard gate**. Resolves fork backlog **BL-5** (the `[BL-5]` BACKLOG removal lands on fork
  ...
  bumps **2.7.0 → 2.8.0**.
- **Commit/PR:** `b2efd76` (dashboard logic, both twins) · this commit (tests + `.gitignore` +
  `bin/tests.sh` wiring + this ledger entry) — branch `feat/dashboard-doc-only-scoring` (from
  `upstream/main`) → upstream PR (pending). Design pressure-tested by a 3-way judge panel +
  adversarial synthesis (workflow `wf_7174281b-754`).
- **Session:** BL-5 dashboard doc-only scoring · **Verified:** 23/23 dashboard unit tests + 54/54
  ...
  synthetic doc-only tree detects doc-only, scores Render/Verify 19/20, and drops the false
  no-test-infra + big-`.md` risks; no `starter-kit/__pycache__` generated by the tests.
