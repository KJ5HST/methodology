...
- **The Plan-Mode exit trap.** When a multi-phase plan arrives in the prompt under Plan Mode's auto-preamble *"Implement the following plan,"* that wording is **not** a go-ahead to code. The plan is a draft until it's written to `docs/planning/` with an evidence-based inventory; planning and implementation are separate sessions. See the [Phase 1 trap warning](../../starter-kit/SESSION_RUNNER.md#phase-1-receive-task), **[FM #19](../../starter-kit/SESSION_RUNNER.md#known-failure-modes)** (plan-mode bypass) and **[FM #18](../../starter-kit/SESSION_RUNNER.md#known-failure-modes)** (planning-to-implementation bleed).
- **The vertical-slice masquerade.** "1 and done" has exactly one sanctioned way to ship more than one layer: a *verified vertical slice* — **one capability** end to end under four mandatory gates (a pre-declared contract, a checkpoint commit at every layer boundary, full verification at every boundary, and faithful per-surface verification). It is never a license to bundle two capabilities. Bundling F2 **and** B1 and calling it "a slice" is **[FM #26](../../starter-kit/SESSION_RUNNER.md#known-failure-modes)**. The test: does one prior-session plan-mode contract enumerate exactly this layer set, all of the *same* capability? See [§Vertical Slice Sessions](../../starter-kit/SESSION_RUNNER.md#vertical-slice-sessions).
