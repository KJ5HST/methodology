...
**The documents the protocol orders you to read now have ceilings** — the first new failure mode since v3.1, and the first new distributed tool since v2.8. The methodology tells every session to *write* a durable record (Phase 3C a learning, Phase 3D a handoff, Phase 3A a predecessor evaluation) and no phase ever tells one to *reduce* one. That is a compounding term with no decay term.

- **New failure mode #28, "Unbounded mandatory read"** — appended, never inserted; FMs 1–27 are byte-unchanged and the **count moves 27 → 28**. Plus four Degradation Detection rows and new **`starter-kit/context_budget.py`**: a stdlib-only size checker with a declarative per-project config, distributed `TRACKED` with a `SEED` config, plus a pre-commit gate.
- **Measured, not theorised** — on adopter project ResortApp across 51 raw session transcripts, opening context rose from **45,931 to 103,241 tokens over 38 consecutive sessions and never once decreased**, reversing only when a human hand-extracted 156 KB out of `CLAUDE.md`. `SESSION_NOTES.md` reached **26,097 lines ≈ 1.02M tokens** — larger than the window Phase 0 mandates reading it into. The measured median session read **0.72% of it**.
- **Why a gate and not a report** — the dashboard already printed `Large files detected` at every Phase 0, and **15+ consecutive sessions read past it**. The signal was never missing; nothing gated on it. So the tool *refuses* a commit that grows a budgeted file past its ceiling, ranks five remedies with "raise the ceiling" deliberately last, and states what `--no-verify` costs.
- **Size hides the refutation, not the false claim** — the claim that cost one session its whole deliverable sat in `CLAUDE.md`, which *is* read in full; the evidence against it sat 503 lines past anything anyone reads.

**New Learning #13 — a forward-looking claim has to be computed, not re-read.** Learning #6 and FM #11 catch claims written from memory and both prescribe "go re-read the file"; a prediction describes a state that does not exist yet, so no file confirms it. Where the answer is computable, compute it — `git merge-tree --write-tree --name-only` lists a merge's conflicting paths without touching a working tree (rmsharp, PR #63). **Phase 3D carries the writer-side half**, since requirements 3 ("what's next") and 5 ("gotchas") are the two that hold predictions — no new requirement; the six are unchanged.

**The repo's own numbered sets now have tests** (issue [#65](https://github.com/KJ5HST/methodology/issues/65)) — Learning #12 applied to the files Learning #12 lives in. A Learning row could be renumbered, duplicated, malformed, or deleted, and an older `HANDOFFS.md` receipt destroyed outright, with every check still green. New **`bin/check-learnings`** asserts the table is contiguous from 1, four columns, one row per line, and that every `Learning #N` cited in the distributed corpus resolves; **`bin/check-handoff --all`** extends the checker past its newest-only blind spot to every receipt, plus fence balance, orphaned bodies, and duplicate receipt identities (`session` + `date` — the pair, since one ledger may merge two `S<N>` sequences). *Canonical-only — these guard this repo's corpus, not an adopter's.*

**Scanner fixes — `DASHBOARD_VERSION` 2.10.2 → 2.10.6**, four of them, three from external review:

- **The installer no longer miscounts its own tools** ([#71](https://github.com/KJ5HST/methodology/pull/71)) — `bin/sync` began installing `context_budget.py` (674 LOC) into adopter roots against a 200-LOC doc-only cap, so **adopting the methodology re-created the exact false "No test infrastructure" penalty v3.2 exists to remove**. Content verification is now **per file**: each installed name carries its own signature set, rather than every name being checked against the *scanner's* signatures — which the first version of the fix did, leaving the exclusion silently never firing while its name-list test passed.
- **R, Quarto and R Markdown are recognized** ([#72](https://github.com/KJ5HST/methodology/pull/72)) — `.r` counted toward Source LOC but had no "Code by Language" row; `.qmd`/`.rmd` were in *neither* the source nor the doc set, so either extension outside a `docs/` path fell through to `"other"` and was not LOC-counted at all. A real R package showed 603 `.r` files / 77,773 LOC as Source and invisible by language. A bare `.Rmd` analysis repo now correctly reads document-only.
- **The doc-only thresholds are pinned** ([#70](https://github.com/KJ5HST/methodology/pull/70)) — recorded as stated heuristics rather than round numbers that read as calibrated, with a regression test so a future edit is deliberate.
- **A remedy proportionate to the finding** (issue [#67](https://github.com/KJ5HST/methodology/issues/67)) — the stale-copy warning answered "this one copy is old" with `--sync`, which is scoped from the canonical's own location and rewrites **26 files across 25 repos**. It now leads with the per-project `cp`. Separately, bare `--dry-run` used to fall through to a full scan and *write* two files; it is now an error. **`--sync`'s blast radius itself is unchanged** — the tool merely stops recommending it.

**Receipt-spec and citation fixes** — the `HANDOFFS.md` spec promised a reconcile no procedure performs (`commit: pending` is a legitimate resting value, not a duty owed) and offered locator forms no receipt uses; the quoted-`###`-heading form all 8 live receipts actually use is now blessed ([#69](https://github.com/KJ5HST/methodology/pull/69)). **Five citations to Learnings that never existed** — `RECOMMENDED_SKILLS.md` and `DEVELOPMENT_WORKSTREAM.md` cited Learnings #28/#29/#30/#34, carried in from a v2.6 audit quoting *rad-con's* numbering; each now states its substance. The `/caveman` row's claim about a "handoff length discipline" is re-grounded on FM #15 and the six Minimum Handoff Requirements, since no length rule exists to cite ([#68](https://github.com/KJ5HST/methodology/pull/68)).

**No principle, phase, gate, or workstream change. The failure-mode count moves 27 → 28** (FM #28 appended); Learnings move 12 → 13. Test suites: `bin/tests.sh` **84 → 114**, dashboard unit suite **197 → 211**.
